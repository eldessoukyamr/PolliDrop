...
// the sturct identifies and sets the parameters in order for the UART construct functions to operate using the input in the identified parameters
struct _UART
{
    UART_Config config;
...
// Returns the character in UART
char UART_getChar(UART* uart_p);
// If it has a character it returns true if it does not then false
bool UART_hasChar(UART* uart_p);
// If it can send a character then true if it does not then false
